...
CodePlayground 本质上是解析 url 上的 `hash` 值，初始化对应的文件及代码，并将代码及运行结果呈现到页面上。

一个 CodePlayground Demo 的访问链接格式应该如下：

```
https://delta-component.woa.com#xxx
```

## 示例

使用 CodePlayground 展示 demo 的核心是将 demo 的代码转换为 url 的 `hash`。接下来我们通过一个案例来了解如何将代码转换为 `hash`，该 demo 包含下列文件：
