...
export const activeClass = 'codeplayground-active-menu-item';
...
export default class RightMenu {
  ...
  initSkeleton(e: MouseEvent): void {
    ...
    const skeleton = this.createDom(
      'ul',
      {
        class: `codeplayground-right-menu-list codeplayground-theme-${this.config.theme}  codeplayground-theme-${this.config.color}`,
   ...
   */
  destroyMenu(): void {
    const menuList = document.querySelectorAll('.codeplayground-right-menu-list');
    // 清除所有菜单栏, 有多少清多少
    menuList &&
  ...
  renderMenu(options: ItemType[]): HTMLElement {
    ...
    return this.createDom(
      'ul',
      {
        class: `codeplayground-right-menu-list codeplayground-theme-${this.config.theme}`,
        style: this.menuStyle,
      },
      children
    );
  ...
  createLi<T extends ItemType & { type: 'li' | 'ul' }>(opt: T): HTMLElement {
    ...
    if (opt.type === 'li' && opt.arrow) {
      ...
      li.classList.add('codeplayground-not-active-menu-item');
    }
